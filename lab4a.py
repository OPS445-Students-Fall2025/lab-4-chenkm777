...
def join_sets(s1, s2):
    """Return a set containing all values from s1 and s2."""
    return s1 | s2  # or s1.union(s2)

def match_sets(s1, s2):
    """Return a set containing values found in both s1 and s2."""
    return s1 & s2  # or s1.intersection(s2)

def diff_sets(s1, s2):
    """Return a set of values NOT shared between s1 and s2 (symmetric difference)."""
